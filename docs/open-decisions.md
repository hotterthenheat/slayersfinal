...
| Number punctuation across locales | every route loaded twice, once with the browser set to `en-US` and once to `de-DE`, comparing what actually rendered | **a real defect, fixed.** 44 `toLocaleString` calls across 41 lines in 13 files took the **viewer's** locale; exactly one passed a locale, and one passed `undefined` explicitly. On a German browser `/trace/live-tape` showed `2.350` and `$512.28` on one screen — the same dot as a thousands separator in one column and a decimal point in the next. It was inconsistent between routes too (`5,135` on Pulse, `2.350` on the tape), which is the tell that nothing was deciding it. All now go through `core/numFormat.ts`, pinned to the locale `etFormat.ts` already pins for dates |
...
| The gate's own assertions | all 180 predicate conditions across the nine proof scripts, read for shapes that cannot fail. Mutation-verified against the real historical defects | **0 tautological.** The recurring defect here is a guard that passes without proving anything — seven have been found, each by hand — so `gate-proof.ts` now scans for the shapes that made them vacuous and fails on any of them. It does **not** claim the other assertions are meaningful: a regex matching the wrong thing looks perfectly healthy to it, and only mutation verification catches that. A first pass with looser rules flagged eight, and all eight were fine — including a "did the scan run" companion check, which is the *antidote* to vacuity and had been flagged as an instance of it |
| Do the counters match the screen | every "N &lt;noun&gt;" caption on six routes, each compared against the rows it claims to count, scoped to the panel that makes the claim | **clean — 5 claims, 0 disagreeing.** The tape's "12 prints on tape" against 12 rows, "6 of 16 prints" against 6 after filtering, Pinpoint's "61 strikes" against 61 cards and ladder rows, Earnings' "14 reports" against 14, and Community's "4 ideas" against 4. This is the shape that goes quietly wrong — the filter applied in one place, the count taken in another — and a stale caption looks exactly like a correct one |
...
- **A bulk rewrite needs the compiler pointed at it, not a green test suite.** The regex that pinned 44 call sites also matched the parenthesised argument of `Math.abs(x).toLocaleString()` and produced `Math.absfmtNum(x)` — six times, in four files. Every assertion still passed, because the assertion asks whether a bare `toLocaleString` survives and none did. `tsc` caught all six in one run. The lesson is not "write a better regex": it is that a mechanical edit across a tree needs a check that reads the *code*, and the type checker is the only thing in this gate that does.
- **A test can encode the wrong expectation and look like a finding.** The ticker-switch edge case asserted that swapping between two same-length pinned recordings keeps the "recording played out" label up — and reported a failure. The code was right and the assertion was wrong: comparing the ticker is exactly what stops a same-length swap reading as a freeze, and the price of that is a reset, so the label must be absent for a moment afterwards. Measured instead of argued: it comes back in **1640–2072ms**, under two ticks, varying only with where the switch lands in the tick phase. The lesson is the same one as the `truncate` false positives — a probe that encodes a preference rather than a property will report the design as a defect.
- **Three probe bugs in one check, and the third was a defect this file already records.** The counter check reported the tape's caption wrong by eight, twice: first because `tbody tr` counted both tables on the route (the Options Tape and the Dark Pool panel beneath it), then because scoping by panel name returns the *outermost* matching element, which wraps both. The third was passing a regex into the page as `re.source` and losing the `i` flag with it — so every case-insensitive caption pattern went case-sensitive against text that renders uppercase, and three routes reported "no caption" when each had one. That is the same defect recorded further down this list, met again from a different direction, which is the argument for writing them down at all.
