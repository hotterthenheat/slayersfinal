...
const Tab = ({ item, pillId }: { item: SubNavItem; pillId: string }) => (
  <NavLink
    to={item.path}
    className={({ isActive }) =>
      `relative shrink-0 px-3 py-1.5 font-mono text-caption leading-4 whitespace-nowrap transition-colors rounded-[5px] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-select/60 active:scale-[0.98] ${
        isActive
          ? 'text-ink font-semibold'
          : 'text-textSecondary font-medium hover:text-textPrimary hover:bg-rowHover rounded-[5px]'
      }`
    }
  >
    {({ isActive }) => (
