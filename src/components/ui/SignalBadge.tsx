...
const SignalBadge = ({ tone = 'neutral', dot = false, pulse = false, children, className = '' }: SignalBadgeProps) => {
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded border px-1.5 py-0.5 font-mono text-label font-semibold uppercase tracking-wider ${toneBadge[tone]} ${className}`}
    >
