...
const TickerTag = ({ symbol, className = '' }: TickerTagProps) => {
  ...
  return (
    <button
      ...
      className={`rounded-[3px] -mx-1 px-1 -my-1.5 py-1.5 cursor-pointer transition-colors hover:bg-select/15 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-select/60 ${className}`}
    >
      {symbol}
    </button>
  );
