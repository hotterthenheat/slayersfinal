...
/*
==================================================
  ONE CLOCK, AND IT IS EASTERN.

  lightweight-charts renders a UNIX timestamp in UTC
  unless told otherwise, so a bar stamped 09:30 ET
  paints as 13:30 on the axis. Shifting the data to
  make UTC read as ET is the usual shortcut and it
  is a lie in the fixture: the epochs stop being the
  moments they claim to be, and everything that
  later joins on them — a real feed most of all —
  joins against a four-hour offset.

  So the data stays true and the AXIS is told which
  zone to speak. Exported from here because every
  chart already imports this module for its theme,
  and a second chart formatting its own ticks is how
  two panels end up on two clocks.
==================================================
*/
...
const ET_DATE = new Intl.DateTimeFormat('en-GB', {
  timeZone: 'America/New_York',
  day: '2-digit',
  month: 'short',
});

/** UNIX seconds → "HH:MM" in Eastern. */
export const etTime = (unixSeconds: number): string => ET_TIME.format(unixSeconds * 1000);

/** UNIX seconds → "24 Aug" in Eastern. */
export const etDate = (unixSeconds: number): string => ET_DATE.format(unixSeconds * 1000);

/**
 * Axis tick text. Intraday ticks read as ET clock times; the first tick of a
 * session reads as the date, which is what makes a multi-day 1m chart legible
 * as separate sessions rather than one continuous run.
 */
export function etTickMark(time: number, tickMarkType: number): string {
  // TickMarkType: 0 Year · 1 Month · 2 DayOfMonth · 3 Time · 4 TimeWithSeconds
  return tickMarkType <= 2 ? etDate(time) : etTime(time);
}
