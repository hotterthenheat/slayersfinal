...
const WallDrift = ({ drift }: WallDriftProps) => {
  ...
  if (drift.length < 2) {
    return (
      <div className="h-40 flex items-center justify-center font-mono text-label text-textMuted uppercase tracking-widest">
  ...
  return (
    <div className="flex flex-col gap-2 h-full min-h-0">
      ...
      <div className="flex items-center gap-3 flex-wrap select-none">
        {SERIES.map(s => (
          <span key={s.label} className="flex items-center gap-1.5 font-mono text-micro uppercase tracking-wider text-textSecondary">
            <span
          ...
          (() => {
            ...
            return (
              <>
                ...
                <div
                  className="absolute top-1 z-10 pointer-events-none border border-borderSubtle bg-panelRaised/95 rounded-md px-2.5 py-2 shadow-overlay min-w-[132px]"
                  style={flipSide ? { right: `${100 - xPct + 1.5}%` } : { left: `${xPct + 1.5}%` }}
                >
                  <div className="font-mono text-micro uppercase tracking-widest text-textMuted tnum mb-1.5">
                    {timeLabel(p.time)}
                  </div>
                  <div className="flex flex-col gap-1">
                    {rows.map(r => (
                      <div key={r.label} className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-micro uppercase tracking-wider text-textSecondary">{r.label}</span>
                        <span className="ml-auto pl-3 font-mono text-micro font-semibold tnum text-textPrimary">
                          {r.v.toFixed(2)}
                        </span>
                      </div>
                    ))}
