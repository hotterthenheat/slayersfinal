      ...
      <div className="flex min-h-[60vh] items-center justify-center px-4">
        <div className="w-full max-w-md rounded-lg border border-borderMuted bg-panel px-6 py-7 flex flex-col items-center text-center gap-4 shadow-overlay">
          ...
          <div className="flex flex-col gap-1.5">
            <h2 className="font-mono text-data font-semibold uppercase tracking-widest text-textPrimary">
            ...
            <p className="mt-1 font-mono text-micro text-textMuted break-words">{error.message || 'Unknown error'}</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => this.setState({ error: null })}
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded border border-borderSubtle bg-white/[0.02] font-mono text-label uppercase tracking-wider text-textSecondary hover:text-textPrimary hover:border-borderMuted transition-colors"
            >
              <RotateCcw className="w-3.5 h-3.5" /> Retry
            </button>
            <a
              href="/pulse"
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded border border-select/30 bg-select/10 font-mono text-label uppercase tracking-wider text-select hover:bg-select/15 transition-colors"
            >
