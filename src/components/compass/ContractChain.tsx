...
const ChainCell = ({ side, right, strike, ticker, isSelected, onSelect }: CellProps) => {
  ...
  return (
    <button
      onClick={onSelect}
      /* Hover = a soft white lift (Noah's pick, 2026-08-09 — more present
         than the old 0.02 wash, quieter than a color); the SELECTED cell
         alone speaks lime, the house selection grammar (DataTable's tint +
         inset edge). The old white selected-inset was nav language on data. */
      className={`text-left px-2.5 py-2 transition-colors ${
        isSelected
          ? 'bg-[#C7D3E8]/[0.07] shadow-[inset_2px_0_0_0_rgba(199,211,232,0.8)]'
          : 'hover:bg-white/[0.04]'
      }`}
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-right leading-tight">
          <span className="block font-mono text-[11px] font-semibold tnum text-textPrimary">${side.premium.toFixed(2)}</span>
          <span className={`block font-mono text-[9px] tnum ${changeUp ? 'text-bull' : 'text-bear'}`}>
            {changeUp ? '+' : ''}{side.changePct}%
          </span>
        </span>
      </div>
      {/* Momentum word + action pill — the health grade behind them is
          engine-internal (Noah, 2026-08-16) */}
      <div className="mt-1 flex items-center justify-between gap-2">
        <span className={`font-mono text-[9px] uppercase tracking-wide ${momentumText[side.momentum]}`}>
          {side.momentum}
        </span>
        <span
...
const ContractChain = ({ data, selected, onSelect }: ContractChainProps) => {
  ...
  return (
    <Panel
      ...
      subtitle="momentum · action · premium"
      flush
