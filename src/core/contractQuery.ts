...
==================================================
  ...
  its value: typed, assumed, missing or unknown. A
  search that silently returns the wrong contract is
  worse than one that says it did not understand, so
  nothing is invented (a strike is never guessed) and
  nothing is rolled forward in silence (a date in the
  past stays in the past and is reported).
==================================================
*/

import { expiryFor, fmtExpiry, isoDate, today, MARKET_HOLIDAYS, type Expiry } from './calendar';

export type SlotState = 'typed' | 'assumed' | 'missing' | 'unknown';
 ...
 * offer a date the pricing math would then have to explain away.
 */
 ...
 * before classification rather than given the classifier a special case.
 */
function tokenize(raw: string): string[] {
  ...
  for (const tok of rough) {
    ...
    out.push(tok.startsWith('$') && RE_STRIKE.test(tok.slice(1)) ? tok.slice(1) : tok);
  }
  return out;
}
...
interface DateResolution {
  /** The calendar date the text asked for, before any session snap. */
  target: Date;
  /** Only set when the year was absent and the month/day is behind us. */
  expired: { label: string; weekday: string; daysAgo: number } | null;
}
 ...
 * next year's than a six-month-old typo.
 */
function resolveBareDate(month: number, day: number, now: Date): DateResolution {
  const base = atMidnight(now);
  let target = new Date(base.getFullYear(), month, day);
  if (calendarDaysBetween(target, base) > 183) {
    target = new Date(base.getFullYear() + 1, month, day);
  }
  const daysAgo = calendarDaysBetween(target, base);
  return {
    target,
    expired:
      daysAgo > 0
...
function resolveExactDate(year: number, month: number, day: number, now: Date): DateResolution {
...
function applyDate(d: Draft, res: DateResolution, now: Date): void {
  if (res.expired) {
    ...
    d.expired = res.expired;
    return;
  }
  d.expiry = { state: 'typed', value: expiryFromDate(res.target, now, d.notes) };
}
 ...
 */
function pairMonthWords(tokens: string[], now: Date, d: Draft): boolean[] {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (d.expiry || d.expired) continue;
    applyDate(
      d,
...
export function parseContractQuery(raw: string, ctx: QueryCtx): ContractQuery {
  ...
  const d: Draft = {
    ...
    expired: null, notes: [], leftovers: [],
  };

  const consumed = pairMonthWords(tokens, now, d);

  for (let i = 0; i < tokens.length; i++) {
    if (consumed[i]) continue;
    const tok = tokens[i];

    // 1. OCC — unambiguous, fills every slot and stops reading.
    const occ = tok.match(RE_OCC);
    if (occ) {
      const [, sym, yy, mm, dd, cp, strike8] = occ;
      d.ticker = ctx.knownTicker(sym)
        ? { state: 'typed', value: sym }
        : { state: 'unknown', raw: sym, suggestions: ctx.suggest(sym).slice(0, 3) };
      ...
      d.expiry = null;
      d.expired = null;
      applyDate(d, resolveExactDate(2000 + Number(yy), Number(mm) - 1, Number(dd), now), now);
      break;
    }

    // 2. RIGHT — beats TICKER, so a bare "C" is a call and not the ticker C.
    if (RE_RIGHT.test(tok)) {
      if (!d.right) d.right = { state: 'typed', value: tok.startsWith('C') ? 'C' : 'P' };
      continue;
    }
    ...
    if (dteM || tok === 'TODAY' || tok === 'TOMORROW') {
      ...
      if (!d.expiry && !d.expired) d.expiry = { state: 'typed', value: expiryFromDte(n, now, d.notes) };
      continue;
    }
    ...
    if (slash || iso) {
      if (!d.expiry && !d.expired) {
        if (iso) {
          applyDate(d, resolveExactDate(Number(iso[1]), Number(iso[2]) - 1, Number(iso[3]), now), now);
        } else if (slash![3]) {
          const y = slash![3].length === 2 ? 2000 + Number(slash![3]) : Number(slash![3]);
          applyDate(d, resolveExactDate(y, Number(slash![1]) - 1, Number(slash![2]), now), now);
        } else {
          applyDate(d, resolveBareDate(Number(slash![1]) - 1, Number(slash![2]), now), now);
        }
      }
      continue;
    }

    // 6. STRIKE — the first bare number, and only the first.
    ...
    if (RE_TICKER.test(tok)) {
      if (ctx.knownTicker(tok)) {
        // A resolved symbol outranks an unresolved one already in the slot.
        if (d.ticker?.state === 'unknown') d.leftovers.push(d.ticker.raw);
        if (!d.ticker || d.ticker.state === 'unknown') {
          d.ticker = { state: 'typed', value: tok };
          continue;
        }
      } else if (!d.ticker) {
        d.ticker = { state: 'unknown', raw: tok, suggestions: ctx.suggest(tok).slice(0, 3) };
        continue;
      }
    }
