...
import { buildStructures, payoffCurve } from '../core/structures';
...
describe('structures: the worst case is arithmetic, not a stop', () => {
  const built = () => buildStructures(snap(), 45);

  it('builds the eight defined-risk shapes', () => {
    const kinds = built().map(x => x.kind);
    expect(new Set(kinds).size).toBe(kinds.length);
    expect(kinds).toContain('iron-condor');
    expect(kinds).toContain('long-straddle');
  });

  it('never risks an unbounded amount', () => {
    // The whole claim of the sleeve. A structure whose loss is not a number does
    // not belong on this board.
    for (const st of built()) {
      expect(Number.isFinite(st.maxLoss), st.label).toBe(true);
      expect(st.maxLoss, st.label).toBeGreaterThan(0);
    }
  });

  it('the straddle family is the only unbounded upside', () => {
    for (const st of built()) {
      const unbounded = !Number.isFinite(st.maxProfit);
      expect(unbounded, st.label).toBe(st.kind === 'long-straddle' || st.kind === 'long-strangle');
    }
  });

  it('the headline numbers are the payoff curve, not a second derivation', () => {
    /*
      Max loss and max profit are read off the same sampled payoff the diagram
      draws. Solving each shape in closed form would be eight special cases free
      to disagree with the picture beside them.
    */
    const s = snap();
    for (const st of built()) {
      const curve = payoffCurve(st, s.spot, 241);
      const lo = Math.min(...curve.map(p => p.profit));
      const hi = Math.max(...curve.map(p => p.profit));
      expect(lo, st.label).toBeGreaterThanOrEqual(-st.maxLoss - 0.51);
      if (Number.isFinite(st.maxProfit)) expect(hi, st.label).toBeLessThanOrEqual(st.maxProfit + 0.51);
    }
  });

  it('a credit structure never claims less profit than the credit it collects', () => {
    /*
      The peak of a butterfly sits exactly on its middle strike, so a sampled
      grid steps past it: measured, a $1,102 credit reported $1,092 of maximum
      profit — the panel disagreeing with itself two lines apart. Extremes come
      off the knots now, which is exact for a piecewise-linear payoff.
    */
    for (const st of built()) {
      if (st.netDebit >= 0) continue;
      expect(st.maxProfit, st.label).toBeGreaterThanOrEqual(Math.abs(st.netDebit) - 0.01);
    }
  });

  it('a credit structure is paid up front and a debit one is not', () => {
    for (const st of built()) {
      const credit = st.kind === 'bull-put' || st.kind === 'bear-call' || st.kind === 'iron-condor' || st.kind === 'iron-butterfly';
      if (credit) expect(st.netDebit, st.label).toBeLessThan(0);
      else expect(st.netDebit, st.label).toBeGreaterThan(0);
    }
  });

  it('probability of profit is a probability', () => {
    for (const st of built()) {
      expect(st.probProfit, st.label).toBeGreaterThanOrEqual(0);
      expect(st.probProfit, st.label).toBeLessThanOrEqual(1);
    }
  });

  it('a wider condor is likelier to pay but pays less for it', () => {
    // The trade-off the sleeve exists to show, held as an invariant.
    const all = built();
    const condor = all.find(x => x.kind === 'iron-condor')!;
    const fly = all.find(x => x.kind === 'iron-butterfly')!;
    expect(condor.probProfit).toBeGreaterThan(fly.probProfit);
